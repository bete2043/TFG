...
export class AbonadoComponent implements OnInit{
  usuarioAutenticado: string | null = null;
    ...
    resumenHistorial: { vecesQuimico: number; vecesOrganico: number; gramosQuimico: number; gramosOrganico: number; gramosTotales: number; } | undefined;
    ...
    obtenerUltimoAbonado(nombreFinca: string, index: number): void {
        ...
        (historial) => {
          if (this.info && this.info[index]) { 
            if (historial.length > 0) {
              ...
              this.info[index].ultimo_abonado = this.formatearFecha(historial[0].fecha);
            } else {
              this.info[index].ultimo_abonado = 'No hay registros';
            }
          }
        },
        (error) => {
